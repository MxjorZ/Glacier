...
import { Save, Server, SlidersHorizontal, FolderTree, ShieldAlert, Radio, Palette, Volume2 } from 'lucide-react';
import { api } from '../api.js';
import { applySettingsTheme, ACCENTS, customAccentVars, parseAccentColor } from '../lib/themes.js';
  ...
  return (
    <div>
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        <Card>
          <CardHeader className="border-b">
            <CardTitle className="flex items-center gap-2"><FolderTree className="size-4 text-primary" /> Organization</CardTitle>
            <CardDescription>Folder and naming layout</CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">Folder pattern</label>
              <Input value={s.folder_pattern || ''} onChange={(e) => set('folder_pattern', e.target.value)} className="font-mono text-xs" />
            </div>
            <div className="space-y-1.5">
              <label className="text-xs text-muted-foreground">Naming pattern</label>
              <Input value={s.naming_pattern || ''} onChange={(e) => set('naming_pattern', e.target.value)} className="font-mono text-xs" />
            </div>
            <div className="space-y-1.5">
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex items-center justify-between rounded-lg border bg-muted/30 px-3 py-2">
              <div>
                ...
                <p className="text-xs text-muted-foreground">Polls Plex every interval; 5 stars → tag rating 100. Newer/larger local ratings win unless overwrite is on.</p>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1.5">
                ...
                <Input type="number" value={s.plex?.rating_sync_interval_sec || 600} onChange={(e) => setNested('plex', 'rating_sync_interval_sec', Number(e.target.value) || 600)} />
              </div>
              <div className="flex items-end pb-2">
                <Switch checked={!!s.plex?.rating_overwrite} onCheckedChange={(v) => setNested('plex', 'rating_overwrite', v)} />
                <span className="ml-2 text-sm">Overwrite higher local rating</span>
              </div>
            </div>
