  ...
  return (
    <header className="fixed left-14 right-0 top-0 z-20 flex h-10 items-center justify-between border-b bg-background/80 px-4 backdrop-blur">
      ...
      <div className="flex items-center gap-3 text-xs text-muted-foreground">
        {errorCount > 0 && (
          <button onClick={onErrors} title="Open Error Center" className="flex items-center gap-1 rounded-full bg-destructive/15 px-2 py-0.5 font-semibold text-destructive hover:bg-destructive/25 transition-all hover:scale-105 hover:shadow-[0_0_20px_var(--destructive)]">
            <AlertTriangle className="size-3.5 animate-pulse" /> {errorCount} error{errorCount === 1 ? '' : 's'}
          </button>
        )}
        {sys && (
          <span className="flex items-center gap-1 font-mono">
            <Server className="size-3.5 animate-pulse" /> {sys.ip}:{sys.port}
          </span>
        )}
      </div>
    </header>
